...
#
import sys
sys.path.insert(0, '/root/autodl-tmp/BLINK-main')
...
def create_samples(input_texts):
    input_text = input_texts.lower()
    samples = []
    record = {
        "label": "unknown", 
        "label_id": "-1", 
...
def run(
    ...
    while not stopping_condition:
        ...
        if args.interactive:
            ...
            ## for entity_list, sample in zip(nns, samples):
                ...
                ## idx += 1
            ## print()
            entity_list, sample = nns, samples
            entity_list_biencoder = []
            for i, e_id in enumerate(entity_list[0]):
                e_title = id2title[e_id]
                e_text = id2text[e_id][:600]
                e_url = id2url[e_id]
                score = scores[0][i]
                entity_list_biencoder.append({"Entity_ID": e_id, "Entity_Title": e_title, "Entity_Text": e_text, "Entity_URL": e_url, "Score": score})
        ...
        print(context_input, candidate_input, label_input)
        ...
        print(accuracy, index_array, unsorted_scores)
        ...
        if args.interactive:
            ...
            idx = 0
            print(nns, index_array, samples)
            
            index_list_reverse = index_array[0][::-1]
            entity_list_crossencoder = []
            for i, e_id_index in enumerate(index_list_reverse):
                e_id = entity_list[0][e_id_index]
                e_title = id2title[e_id]
                e_text = id2text[e_id][:600]
                e_url = id2url[e_id]
                score = unsorted_scores[0][e_id_index]
                entity_list_crossencoder.append({"Entity_ID": e_id, "Entity_Title": e_title, "Entity_Text": e_text, "Entity_URL": e_url, "Score": score})
                idx += 1
            print()
            stopping_condition = True
    ...
    return entity_list_biencoder, entity_list_crossencoder

def extract_and_process_text(row_text, level):
    if level == 1:
        # 提取所有大括号{}内的文本
        texts = re.findall(r'\{(.*?)\}', row_text)
    ...
    flat_list = [item for sublist in processed_texts for item in sublist]
    return flat_list
...
if __name__ == "__main__":
    ...
    for level in range(3, 4):
        ...
        for _, row in df.iterrows():
            ...
            recommended_entities_biencoder = []
            recommended_entities_crossencoder = []
            texts_entities = []
            for text in extracted_texts:
                entity_list_biencoder, entity_list_crossencoder = run(text, args, logger, *models)

                # 分别处理两个解码器的结果并创建对应的实体列表
                biencoder_entities = [{
                ...
                } for entity in entity_list_biencoder]

                crossencoder_entities = [{
                    "Entity_ID": int(entity["Entity_ID"]),
                    "Entity_Name": entity["Entity_Title"],
                    "Entity_Text": entity["Entity_Text"],
                    "Entity_URL": entity["Entity_URL"],
                    "Score": float(entity["Score"])
                } for entity in entity_list_crossencoder]
    ...
    with open('/root/autodl-tmp/Sample/ipc_of_lack/new_ipc_entities_level_3.json', 'w', encoding='utf-8') as file:
        # 使用indent参数
        json_record = json.dumps(ipc_entities, ensure_ascii=False, indent=4)
        file.write(json_record + '\n')

    print("文件保存成功。文件路径: /root/autodl-tmp")
